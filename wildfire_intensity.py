...
#----------------------------------------------------------------------------------
# SCRIPT NAME:  wildfire_intensity.py
#               v.2026.0313
#
...
try:
    ...
    logFile = fp(fpath, 'wildfire_intensity_log.txt')
    log = open(logFile, 'a')
    log.write('\n\n' + '=' * 89 + '\nTool Started on: ' +
        ...
        if CheckArcInfo() == "yes" and CheckSpatialExt() == "yes":
            ...
            for rstYear in range(start_year, end_year + 1):
                ...
                if arcpy.Exists(rst_fire):
                    ...
                    #==============================================================
                    # -------------------------------------------------------------
                    # Step 1: combine intenstiy rasters to determine which cells
                    ...
                    display(' ... Combining intensity rasters', log)
                    # combine intensity rasters
                    rst_intxsalv = arcpy.sa.Combine([rst_intensity, salvage])
                    ...
                    # -------------------------------------------------------------
                    # Step 2: combine fire bondary with intensity values
                    # -------------------------------------------------------------
                    display(' ... Combining fire and intensity rasters', log)

                    rst_combined = arcpy.sa.Combine([rst_fire, rst_intxsalv])

                    display(' ... Adding new fields to output table', log)
                    # add output fields
##                    fld_desc = [['AREA_KM2', 'FLOAT'],
##                                ['HERD_YEAR_FIREID', 'TEXT', '', 50],
##                                ['HERD', 'TEXT', '', 20],
##                                ['PROV', 'TEXT', '', 20],
##                                ['YEAR', 'SHORT'],
##                                ['FIREID', 'SHORT'],
...
##                                ['SALVAGE', 'SHORT']]
                    ...
                    display(' ... Joining intensity table and calculating values', log)
                    # join tables - with wildfire intensity raster
                    join_table = arcpy.management.AddJoin(
                        in_layer_or_view = 'rst_combo_lyr',
                        in_field = rst2Name,
                    ...
                    fldList = ['INTENSITY', 'SALVAGE']
                    exprList = ['!{}!'.format(jointbl_prefix + '.INTENSITY'),
